...
It assumes you'll be using NordVPN with the OpenVPN protocol. If you want to use another VPN provider, adjust Gluetun's configuration in your `simplarr-stack.env` file. Refer to [Gluetun's documentation](https://github.com/qdm12/gluetun-wiki/tree/main/setup/providers) for instructions.
...
1. Decide whether you want to use Plex or Jellyfin as your media server. If you don’t already have a Plex Pass lifetime subscription, Jellyfin is generally the recommended option. Plex or Jellyfin must be deployed before the ARR-Stack to complete the setup.
...
- `Settings > Apps`
  ...
  - Choose **Sonarr**
    - Click the `Advanced settings shown` gear next to the `Test` button
    - **Name**: e.g., `Sonarr`
